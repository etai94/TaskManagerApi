...
def generate_unique_username():
    ...
    return f"testuser_{uuid.uuid4().hex[:8]}"  # Using first 8 chars of UUID
...
def user_and_token(client):
    ...
    assert response.status_code == 200, f"User registration failed. Status: {response.status_code}, Response: {response.content}"
    ...
    assert response.status_code == 200, f"Login failed. Status: {response.status_code}, Response: {response.content}"
...
def test_register(client):
    ...
    """
    username = generate_unique_username()
    register_data = {"username": username, "password": "123qweQWE"}
...
def test_login(client, user_and_token):
    ...
    """
    login_data = {
        "username": user_and_token["username"],
        "password": "123qweQWE",
        "grant_type": "password"
    }
    response = client.post(f"{BASE_URL}/login", data=login_data)
